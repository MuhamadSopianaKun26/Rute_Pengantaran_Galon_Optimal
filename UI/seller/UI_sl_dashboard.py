...
class OrderCard(QFrame):
    ...
    def _build_ui(self):
        ...
        menu.addAction("Terima").triggered.connect(self._on_deliver)
        menu.addAction("Tolak").triggered.connect(self._on_reject)
        kebab.setMenu(menu)
    ...
    def _on_queue(self):
        """Set status menjadi 'menunggu' (siap diantar nanti)."""
    ...
    def _on_deliver(self):
        """Set status menjadi 'sedang_disiapkan' (pesanan diterima dan disiapkan)."""
        if self._confirm("Konfirmasi Terima", "Terima pesanan dan ubah status menjadi Sedang Disiapkan?"):
            self._update_order_status("sedang_disiapkan")
...
class CustomerDashboard(QWidget):
    ...
    def reload_orders(self):
        """Muat ulang data pesanan: Dashboard hanya menampilkan status 'menunggu'."""
        all_orders = load_orders() or []
        self._orders = [o for o in all_orders if (o.get("status") or "").strip().lower() == "menunggu"]
        # Bersihkan container
